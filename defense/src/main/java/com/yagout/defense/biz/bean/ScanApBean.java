package com.yagout.defense.biz.bean;

import java.io.Serializable;

public class ScanApBean implements Serializable{
	 private String mac;

	    private Integer identity;

	    private Integer channel;

	    private Integer signal;

	    private Integer privacy;

	    private Integer encryptWay;

	    private Integer authWay;

	    private Integer wps;

	    private String ssid;

	    private Integer broadcastSsid;

	    private String maker;

	    public String getMac() {
	        return mac;
	    }

	    public void setMac(String mac) {
	        this.mac = mac == null ? null : mac.trim();
	    }

	    public Integer getIdentity() {
	        return identity;
	    }

	    public void setIdentity(Integer identity) {
	        this.identity = identity;
	    }

	    public Integer getChannel() {
	        return channel;
	    }

	    public void setChannel(Integer channel) {
	        this.channel = channel;
	    }

	    public Integer getSignal() {
	        return signal;
	    }

	    public void setSignal(Integer signal) {
	        this.signal = signal;
	    }

	    public Integer getPrivacy() {
	        return privacy;
	    }

	    public void setPrivacy(Integer privacy) {
	        this.privacy = privacy;
	    }

	    public Integer getEncryptWay() {
	        return encryptWay;
	    }

	    public void setEncryptWay(Integer encryptWay) {
	        this.encryptWay = encryptWay;
	    }

	    public Integer getAuthWay() {
	        return authWay;
	    }

	    public void setAuthWay(Integer authWay) {
	        this.authWay = authWay;
	    }

	    public Integer getWps() {
	        return wps;
	    }

	    public void setWps(Integer wps) {
	        this.wps = wps;
	    }

	    public String getSsid() {
	        return ssid;
	    }

	    public void setSsid(String ssid) {
	        this.ssid = ssid == null ? null : ssid.trim();
	    }

	    public Integer getBroadcastSsid() {
	        return broadcastSsid;
	    }

	    public void setBroadcastSsid(Integer broadcastSsid) {
	        this.broadcastSsid = broadcastSsid;
	    }

	    public String getMaker() {
	        return maker;
	    }

	    public void setMaker(String maker) {
	        this.maker = maker == null ? null : maker.trim();
	    }

		
	    
}
